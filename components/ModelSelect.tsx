  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      <button 
        ...
        className="w-full flex items-center justify-between px-4 py-3 bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl hover:border-zinc-400 dark:hover:border-zinc-500 transition-colors text-sm text-left"
      >
        ...
        <div className="absolute z-50 w-full mt-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-2xl overflow-hidden">
          ...
          <div className="p-3 border-b border-zinc-200 dark:border-zinc-800">
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-8 pr-4 py-2 text-sm bg-zinc-50 dark:bg-zinc-800 rounded-lg focus:outline-none focus:ring-1 focus:ring-purple-500 text-zinc-900 dark:text-white placeholder-zinc-500"
                autoFocus
              />
            </div>
          </div>
          ...
          <div className="max-h-60 overflow-y-auto p-2 space-y-1">
            {filteredModels.map(model => (
              <button
                key={model.id}
                onClick={() => { setSelectedModel(model.id); setIsOpen(false); setSearch(''); }}
            ...
            {filteredModels.length === 0 && (
              <p className="text-sm text-zinc-500 text-center py-4">No models found.</p>
            )}
          </div>
        </div>
      )}

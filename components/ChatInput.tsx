...
import { aiModels, quickModes } from "@/lib/models";
...
export default function ChatInput() {
  ...
  const { sendMessage, selectedModel, setQuickMode, isTyping } =
    useAppContext();
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type.startsWith("image/")) {
      ...
      setIsParsing(true);
      try {
        // FIX: Dynamically import pdfjs-dist
        const pdfjsLib = await import("pdfjs-dist");

        // CHANGED: Point to the local worker file in the public folder
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {attachment && (
          ...
          >
            ...
            />
            <span className="text-xs text-zinc-700 dark:text-zinc-300 max-w-[100px] truncate">
              {attachment.name}
            </span>
            <button
          ...
          <div className="flex items-end p-2 gap-1">
            ...
            <div>
              <input
                ...
                className="hidden"
                accept="image/*,.pdf,.txt,.md,.js,.ts,.py,.html,.css,.json,.xml,.yaml,.yml,.csv"
              />
              <button
                onClick={() => fileInputRef.current?.click()}
                disabled={isParsing}
                className="p-2.5 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors text-zinc-500 dark:text-zinc-400 disabled:opacity-50"
              >
                {isParsing ? (
            ...
            <textarea
              ...
              placeholder={
                isParsing ? "Extracting PDF text..." : "Type a message..."
              }
              rows={1}
              disabled={isParsing}
              className="flex-1 bg-transparent text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-zinc-600 resize-none py-2.5 px-3 focus:outline-none text-sm max-h-[120px] overflow-y-auto disabled:opacity-50"
            />
            ...
            <div>
              <button
                ...
                disabled={
                  isTyping || isParsing || (!input.trim() && !attachment)
                }
              >
                <Send size={18} />
              </button>

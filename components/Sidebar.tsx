...
import { Plus, Flame, MessageSquare, X, Trash2, User } from 'lucide-react';

export default function Sidebar({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  const { chats, activeChatId, setActiveChatId, startNewChat, deleteChat, user } = useAppContext();
    ...
    <>
      ...
      >
        ...
        <div className="p-4 flex justify-between items-center border-b border-black/10 dark:border-white/10">
          ...
          <button onClick={onClose} className="p-1 hover:bg-black/10 dark:hover:bg-white/10 rounded-lg transition-colors text-zinc-600 dark:text-zinc-300"><X size={20} /></button>
        </div>

        {/* Action Buttons */}
        <div className="p-4 space-y-2">
          <button 
            onClick={() => { startNewChat(false); onClose(); }}
          ...
          <button 
            onClick={() => { startNewChat(true); onClose(); }}
            className="w-full flex items-center gap-2 px-4 py-3 rounded-xl border border-zinc-300 dark:border-white/20 hover:bg-black/5 dark:hover:bg-white/10 transition-all active:scale-95 text-zinc-800 dark:text-zinc-200"
          >
            <Flame size={18} /> Temp Chat
          </button>
        </div>
          ...
          {chats.map(chat => (
            <div key={chat.id} className="group relative">
              ...
              >
                ...
                {chat.isTemp && <span className="ml-auto text-[10px] bg-orange-100 text-orange-600 dark:bg-orange-500/20 dark:text-orange-400 px-2 py-0.5 rounded-full flex-shrink-0">TEMP</span>}
              </button>
              {/* Delete Button */}
              <button 
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-zinc-500 dark:text-zinc-400">
                {user ? 'Puter Account' : 'Sign in to save'}
              </p>
            </div>
          </div>
        </div>
